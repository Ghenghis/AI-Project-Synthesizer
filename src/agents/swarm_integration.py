...
class SwarmIntegration:
    ...
    def __init__(
        ...
        if SWARM_AVAILABLE:
            ...
            secure_logger.warning(
                "Swarm not installed. Run: pip install git+https://github.com/openai/swarm.git"
            )
    ...
    async def decompose_task(self, task_description: str) -> list[dict[str, str]]:
        """
        ...
        for line in lines:
            ...
            if line and (line[0].isdigit() or line.startswith("-")):
                ...
                steps.append({"step": step, "agent": agent})

        return steps

    async def generate_docs(self, code: str, doc_type: str = "docstring") -> str:
        """
        ...
        result = await self.quick_handoff(agent_name="doc_generator", message=message)

        return result.response if result.success else ""

    async def quick_fix(self, code: str, error_message: str | None = None) -> str:
        """
        ...
        result = await self.quick_handoff(agent_name="quick_fixer", message=message)
    ...
    async def get_agent_list(self) -> list[dict[str, str]]:
        ...
        return [
            {
                "name": "code_helper",
                "description": "Quick coding questions and answers",
            },
            {
                "name": "doc_generator",
            ...
            {
                "name": "complex_reviewer",
                "description": "Handle complex analysis and architecture",
            },
        ]
