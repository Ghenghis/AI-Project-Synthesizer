...
class ProjectAssembler:
    ...
    def _init_credentials(self):
        ...
        self.config.github_username = (
            settings.platforms.github_token.get_secret_value()[:20]
    ...
    def _generate_project_name(self, idea: str) -> str:
        ...
        return (
            "-".join(name_words)
            if name_words
            else f"project-{datetime.now().strftime('%Y%m%d')}"
        )
    ...
    async def _download_github_repo(
        self, project: AssembledProject, resource: ProjectResource
    ):
        ...
        try:
            ...
            secure_logger.info(
                f"Cloning {resource.metadata.get('full_name', resource.name)}..."
            )
    ...
    async def _download_huggingface_model(
        ...
        try:
            ...
            secure_logger.info(
                f"Downloading model {resource.metadata.get('full_name', resource.name)}..."
            )
    ...
    async def _download_kaggle_dataset(
        self, project: AssembledProject, resource: ProjectResource
    ):
        ...
        try:
            ...
            secure_logger.info(
                f"Downloading dataset {resource.metadata.get('full_name', resource.name)}..."
            )
...
"""
    ...
    def _generate_windsurf_config(self, project: AssembledProject) -> dict[str, Any]:
        ...
        return {
            ...
            "sources": {
                "code_repos": [
                    r.metadata.get("full_name", r.name) for r in project.code_repos
                ],
                "models": [m.metadata.get("full_name", m.name) for m in project.models],
                "datasets": [
                    d.metadata.get("full_name", d.name) for d in project.datasets
                ],
            },
            "tasks": [
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status == 201:
                        ...
                        secure_logger.info(
                            f"Created GitHub repo: {project.github_repo_url}"
                        )
                    else:
    ...
    async def _init_and_push_git(self, project: AssembledProject, clone_url: str):
        ...
        commands = [
            ...
            [
                ...
                "commit",
                "-m",
                "Initial commit - assembled by AI Project Synthesizer",
            ],
            ["git", "branch", "-M", "main"],
