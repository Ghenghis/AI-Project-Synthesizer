...
class ProjectClassifier:
    ...
    def _detect_technology_stack(self, files: list[Path]) -> TechnologyStack:
        ...
        for file_path in files:
            ...
            for tool, patterns in self.deployment_patterns.items():
                if any(
                    pattern in file_name or pattern in content for pattern in patterns
                ):
    ...
    async def _classify_project_type(
        self, files: list[Path], stack: TechnologyStack, structure: dict[str, int]
    ) -> ProjectType:
        ...
        # Web API indicators
        if any(
            fw in stack.frameworks
            for fw in ["fastapi", "flask", "express", "spring", "django"]
        ) and any("api" in name or "route" in name for name in file_names):
            return ProjectType.WEB_API

        # Web App indicators
        if any(
            fw in stack.frameworks for fw in ["react", "vue", "angular", "next"]
        ) and any("public" in path or "static" in path for path in file_paths):
            return ProjectType.WEB_APP

        # Mobile App indicators
        if any(
            indicator in " ".join(file_paths)
            for indicator in [
                "android",
                "ios",
                "react-native",
        ...
        # CLI Tool indicators
        if any("main" in name and "cli" in name for name in file_names) or any(
            "argparse" in path or "click" in path or "typer" in path
        ...
        if (
            any(
                "setup.py" in name or "pyproject.toml" in name or "package.json" in name
                for name in file_names
            )
            and structure["total_files"] < 100
            and structure["test_files"] > 0
        ):
        ...
        if any(
            fw in stack.frameworks
            for fw in ["pandas", "spark", "airflow", "dagster", "luigi"]
        ):
        ...
        if any(
            fw in stack.frameworks
            for fw in ["tensorflow", "pytorch", "sklearn", "keras", "jax"]
        ):
            return ProjectType.MACHINE_LEARNING

        # Microservice indicators
        if (
            any("service" in name for name in file_names)
            and any(fw in stack.frameworks for fw in ["fastapi", "flask", "express"])
            and "docker" in stack.deployment
        ):
        ...
        if any(
            indicator in " ".join(file_paths)
            for indicator in ["unity", "unreal", "pygame", "godot", "game"]
        ):
        ...
        if any(
            indicator in " ".join(file_paths)
            for indicator in ["arduino", "raspberry", "embedded", "firmware", "mqtt"]
        ):
    ...
    def _assess_complexity(
        self, structure: dict[str, int], _files: list[Path]
    ) -> ComplexityLevel:
    ...
    def _detect_architecture(
        self, files: list[Path], patterns: list[str], stack: TechnologyStack
    ) -> ArchitecturePattern:
        ...
        # MVC pattern
        if all(p in patterns for p in ["model", "view", "controller"]) or all(
            p in " ".join(file_names) for p in ["model", "view", "controller"]
        ):
            return ArchitecturePattern.MVC

        # MVVM pattern
        if "viewmodel" in " ".join(file_names) or "ViewModel" in " ".join(file_paths):
            return ArchitecturePattern.MVVM

        # Clean Architecture
        if any(
            layer in " ".join(file_paths)
            for layer in ["entities", "use_cases", "interfaces", "frameworks"]
        ) or "clean" in " ".join(file_paths):
            return ArchitecturePattern.CLEAN_ARCHITECTURE

        # Hexagonal
        if any(
            port in " ".join(file_paths)
            for port in ["ports", "adapters", "infrastructure"]
        ) or "hexagonal" in " ".join(file_paths):
            return ArchitecturePattern.HEXAGONAL

        # Microservices
        if (
            len([f for f in file_names if "service" in f]) > 1
            and "docker" in stack.deployment
        ):
        ...
        if (
            any(
                layer in " ".join(file_paths)
                for layer in ["layers", "repository", "service", "controller"]
            )
            and "repository" in patterns
        ):
        ...
        if (
            any(
                event in " ".join(file_paths)
                for event in ["event", "message", "queue", "publisher", "subscriber"]
            )
            or "kafka" in stack.tools
            or "rabbitmq" in stack.tools
        ):
        ...
        if (
            any(
                serverless in " ".join(file_paths)
                for serverless in ["lambda", "function", "serverless", "faas"]
            )
            or "serverless" in stack.deployment
        ):
        ...
        if (
            any(
                plugin in " ".join(file_paths)
                for plugin in ["plugin", "extension", "module", "addon"]
            )
            and len([f for f in file_names if "plugin" in f or "module" in f]) > 0
        ):
    ...
    def _detect_conventions(
        self, files: list[Path], _stack: TechnologyStack
    ) -> list[str]:
        ...
        if js_files:
            # Check for camelCase
            camel_case = sum(
                1 for f in js_files if any(c.isupper() for c in f.name[1:-3])
            )
    ...
    def _assess_quality_indicators(
        self, files: list[Path], structure: dict[str, int]
    ) -> dict[str, Any]:
        ...
        indicators = {
            ...
            "has_ci_cd": any(
                "ci" in str(f).lower() or "workflow" in str(f).lower() for f in files
            ),
    ...
    async def _save_classification(
        self, characteristics: ProjectCharacteristics, project_path: str
    ) -> None:
        ...
        await self.memory.add(
            ...
            tags=[
                ...
                characteristics.complexity.value,
            ],
            importance=0.7,
        )
    ...
    def get_recommendations(self, characteristics: ProjectCharacteristics) -> list[str]:
        ...
        if characteristics.type == ProjectType.WEB_API:
            ...
            if not any(
                fw in characteristics.stack.frameworks
