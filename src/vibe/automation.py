...
class VibeAutomation:
    ...
    def __init__(self, project_root: Path | None = None, config: AutomationConfig | None = None):
    ...
    async def on_file_change(self, file_path: Path) -> list[AutomationEvent]:
        ...
        # Auto-lint
        if self.config.auto_lint and self.config.mode in [AutomationMode.AGGRESSIVE, AutomationMode.FULL_AUTO]:
        ...
        # Auto-format
        if self.config.auto_format and self.config.mode in [AutomationMode.AGGRESSIVE, AutomationMode.FULL_AUTO]:
...
async def main():
    ...
    parser.add_argument("command", choices=["health", "fix", "test", "watch", "workflows"])
    parser.add_argument("--file", help="Specific file to process")
    parser.add_argument("--mode", choices=["passive", "assisted", "aggressive"], default="assisted")
    args = parser.parse_args()
    
    automation = VibeAutomation(
        config=AutomationConfig(mode=AutomationMode(args.mode))
    )
