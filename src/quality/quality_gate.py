...
from src.quality.security_scanner import SecurityScanner, ScanResult, SecurityIssue, SecuritySeverity
from src.quality.lint_checker import LintChecker, LintResult, LintIssue
from src.quality.test_generator import TestGenerator, TestSuite
from src.quality.review_agent import ReviewAgent, ReviewReport, ReviewSeverity
...
class GateResult:
    ...
    security_result: Optional[ScanResult] = None
...
class QualityGate:
    ...
    async def evaluate(self, code: str, context: Dict[str, Any], auto_fix: bool = True) -> GateResult:
        ...
        # Check pass criteria
        passed = self._check_pass_criteria(results, metrics)
    ...
    async def evaluate_directory(self, directory: Path, patterns: List[str] = None) -> Dict[str, GateResult]:
        ...
        for i in range(0, len(files), batch_size):
            ...
            for (file_path, _), result in zip(tasks, batch_results):
