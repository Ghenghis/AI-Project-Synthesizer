...
"""
    ...
    async def _generate_python_tests(self, code: str, file_path: str) -> TestSuite:
        ...
        try:
            ...
            # Create test suite
            test_file_path = str(
                Path(file_path).with_name(f"test_{Path(file_path).stem}.py")
            )
            ...
            return TestSuite(
                ...
                coverage_estimate=self._estimate_coverage(
                    functions, classes, test_functions
                ),
            )
    ...
    async def _generate_js_tests(self, code: str, file_path: str) -> TestSuite:
        ...
        for class_info in classes:
            for method in class_info["methods"]:
                tests = await self._generate_function_tests(
                    method, "javascript", class_info["name"]
                )
                test_functions.extend(tests)
        ...
        # Create test suite
        test_file_path = str(
            Path(file_path).with_name(f"{Path(file_path).stem}.test.js")
        )
    ...
    async def _generate_function_tests(
        self, func_info: dict[str, Any], language: str, _class_name: str | None = None
    ) -> list[TestFunction]:
        """Generate tests for a specific function using LLM."""
        # Build prompt for LLM
        target = (
            f"{_class_name}.{func_info['name']}" if _class_name else func_info["name"]
        )
...
{{
    ...
    def _determine_python_imports(
        self, functions_and_classes: list[dict[str, Any]]
    ) -> list[str]:
        ...
        if any(
            f.get("is_async", False) for f in functions_and_classes if "is_async" in f
        ):
    ...
    async def run_tests(self, test_file_path: str) -> dict[str, Any]:
        ...
        try:
            result = subprocess.run(
                cmd, capture_output=True, text=True, timeout=60, cwd=test_file.parent
            )
