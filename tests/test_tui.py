...
class TestTUIViews:
    ...
    async def test_dashboard_view(self, mock_prompt, mock_console, tui):
        ...
        with patch(
            "src.core.health.check_health", new_callable=AsyncMock
        ) as mock_health:
            mock_health.return_value = Mock(
                components=[], overall_status=Mock(value="healthy")
            )
    ...
    async def test_search_view_empty_query(
        self, mock_confirm, mock_prompt, mock_console, tui
    ):
        mock_prompt.ask.return_value = ""
    ...
    @patch("src.tui.app.Prompt")
    @patch("src.tui.app.Confirm")
    async def test_assemble_view_empty_idea(
        self, mock_confirm, mock_prompt, mock_console, tui
    ):
        mock_prompt.ask.return_value = ""
